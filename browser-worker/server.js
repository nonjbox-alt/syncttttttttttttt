...
async function handleAction(session, action) {
  ...
  switch (action.type) {
    case 'navigate':
      await session.page.goto(String(action.url), { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(() => {});
      break;
    ...
    case 'click':
      await session.page.mouse.click(Number(action.x) || 0, Number(action.y) || 0, { button: action.button === 'right' ? 'right' : 'left' });
      break;
    case 'wheel':
      await session.page.mouse.wheel(Number(action.deltaX) || 0, Number(action.deltaY) || 0);
      break;
    case 'key':
      if (action.key) await session.page.keyboard.press(String(action.key));
      break;
    case 'type':
      if (typeof action.text === 'string') await session.page.keyboard.insertText(action.text);
      break;
    case 'setViewport':
      await session.page.setViewportSize({ width: Math.max(640, Number(action.width) || 1280), height: Math.max(360, Number(action.height) || 720) });
      break;
  }
...
wss.on('connection', async (ws, req) => {
  ...
  try {
    ...
    ws.on('message', async raw => {
      try {
        await handleAction(session, JSON.parse(raw.toString()));
      } catch (error) {
        ws.send(JSON.stringify({ type: 'error', message: error?.message || 'browser action failed' }));
      }
    });
