...
export const FirefoxMediaBridge: React.FC = () => {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if (event.data.type === 'SOURCE_MEDIA_READY' || event.data.type === 'SOURCE_MEDIA_EVENT') {
        ...
        if (event.data.type === 'SOURCE_MEDIA_EVENT' && isController) {
  ...
  }, [isController, pauseVideo, playVideo, seekVideo, setVideoPlaybackRate]);
  ...
  useEffect(() => {
    ...
    window.postMessage({ source: 'syncroom-web', type: 'SYNCROOM_EXTENSION_COMMAND', payload: { action: 'RATE', playbackRate: videoState.playbackRate } }, '*');
    ...
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-black text-white p-6">
      <div className="w-full max-w-lg rounded-3xl border border-sky-400/20 bg-slate-950/90 backdrop-blur-xl p-6 shadow-2xl">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-11 h-11 rounded-2xl bg-sky-400/10 border border-sky-400/20 flex items-center justify-center"><Radio className="w-5 h-5 text-sky-300" /></div>
          <div><div className="font-bold">Firefox Media Bridge</div><div className="text-xs text-emerald-300 flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> Connected</div></div>
        </div>
        ...
        <p className="text-xs text-slate-400 leading-5">The video is playing in your paired Firefox source tab. SyncRoom never receives the private M3U8 URL, cookies, page URL, or media bytes.</p>
      </div>
    </div>
  );
};
